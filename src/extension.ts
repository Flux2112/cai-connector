...
async function ensureLoggedIn(
  ...
  if (!cmlUrl) {
    vscode.window.showErrorMessage(
      "CML URL is not configured. Set it in Settings → CAI Connector → CML URL.",
    );
    return false;
  }
  const username = (process.env["USERNAME"] || os.userInfo().username).toLowerCase();
...
async function killOrphanedHelperProcesses(output: vscode.OutputChannel): Promise<number> {
  try {
...
async function killOrphanedEndpointProcesses(output: vscode.OutputChannel): Promise<number> {
  try {
    const result = await new Promise<string>((resolve, reject) => {
      cp.exec(
