...
| [`packages/cli`](packages/cli) | `@defysoftware/cai` on npm | The `cai` command line interface: an agent-first client for the Cloudera AI API v2 — reads plus safe writes, never a delete, plus `cai session` for SSH endpoints on Windows. JSON on stdout, errors as JSON on stderr, exit codes you can branch on. See its [README](packages/cli/README.md). |
...
are in [`docs/plans/cml-api-v2-cli.md`](docs/plans/cml-api-v2-cli.md).
