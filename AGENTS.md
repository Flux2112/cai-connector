...
docs/  AGENTS.md  CLAUDE.md  .npmrc  .github/  .vscode/
```
...
Five non-obvious things about that workflow:
...
- **`npm version --workspaces` does not rewrite inter-workspace dependency ranges.** Verified, not assumed: it bumps each package's own version and leaves `cli`'s pin on `cai-core` at the previous value, which would publish a CLI depending on a `cai-core` version that was never published. The `npm pkg set` line after the bump is what fixes it, and the `node -e` check after that fails the build if the four versions ever fall out of step. It is worse than a stale range, though: with `workspaces-update` at its default, npm re-resolves the tree the instant the bump lands — while the pin is still on the old version, which the freshly bumped workspace no longer satisfies — so it asks the registry for a version that was never published and the step dies with `ETARGET` before anything can correct the pin. Hence `--no-workspaces-update` on the bump and an `npm install --package-lock-only` after the pin is right. That refresh is not optional: the lockfile records each workspace's version, and shipping a tag whose lockfile disagrees with the manifests next to it means `npm ci` fails for whoever checks it out.
- **The npm pre-flight is `npm publish --dry-run`, not `npm whoami`.** Checked against npm 11's source: `publish.js` calls the OIDC token exchange *before* it branches on `--dry-run`, so a dry run genuinely proves the trusted publisher is configured. `npm whoami` goes through `getIdentity`, which only reads a token from `.npmrc` — with trusted publishing there is no token, so it would fail every time and prove nothing. It sits **after the version bump and before the push**, and both halves matter. After the bump, because a dry run of an already published version dies with `EPUBLISHCONFLICT` — before the bump the version is exactly the one the previous release published, so this step passes once and then fails on every release after it. That is precisely how it behaved: green at 1.18.0 when nothing had been published, red at 1.19.0 the moment there was. Before the push, because that is what the guard is for — the bump is still only a local file edit, so a broken credential leaves no pushed commit and no tag pointing at a release that never happened. The Marketplace check can stay ahead of the bump because it only asks for a token and never mentions a version.
...
- **`package-lock.json` must resolve every package from `registry.npmjs.org`; a committed `.npmrc` pins that and a CI step verifies it.** `npm install` run on a machine whose `.npmrc` points at a private mirror rewrites the `resolved` URLs to that mirror; the runner cannot reach the host, so `npm ci` fails on a TLS error that names a certificate rather than the mirror, at step 4 of 18 with nothing else to go on. It is invisible locally — on that machine the host resolves — and it broke the 1.19.0 release, which is why the grep now runs *before* `npm ci` and says what to do. The repository's own `.npmrc` sets `registry=https://registry.npmjs.org/`, so an install here resolves publicly whatever the machine is pointed at, and the grep is the backstop for a lockfile that predates it or arrives from elsewhere. To repair one by hand, rewrite the mirror prefix out of the `resolved` URLs: the tarballs are byte-identical, so the `integrity` hashes stay valid and `npm ci` verifies them anyway.

  **Do not add a `cafile` to that `.npmrc`.** `cafile` *replaces* Node's root store instead of extending it, so a corporate-only PEM makes `registry.npmjs.org` unverifiable — `UNABLE_TO_GET_ISSUER_CERT_LOCALLY`, verified 2026-08-18 — and a path that does not exist on the machine reading it, which is any Windows path on the Linux runner, fails identically. `NODE_EXTRA_CA_CERTS` is the setting that appends, so it covers internal and public hosts at once; on this network it must name a chain PEM carrying the issuing intermediate, not the root alone. `http://ca.oenb.at/chain/oenb-ca-chain.pem` always serves a current one.
...
The safe-writes rule is **not** enforced here. `core` exposes `delete` because it is a client for the whole API; the fence is the CLI's command surface, the same way the extension's safety rule is enforced by never passing `cdswctl`'s blanket `/a` flag. What that means concretely: job creation and update, job runs, applications and file uploads have wrappers; `CreateApplication`, `DeleteApplication` and every other destructive operation deliberately have none, because an unused wrapper is an invitation.

**`updateJob`'s options type is deliberately narrower than the `Job` schema**, and the omissions are the whole point. `UpdateJob` (`PATCH …/jobs/{job.id}` — note the dot; every other job path uses `{job_id}`) is a genuine partial update despite declaring no field mask, but three of the fields it accepts it then ignores: `paused`, `timezone` and the recipient lists come back unchanged on the 200 that reported success, verified against a live instance on 2026-08-27. There is no pause operation anywhere in API v2 either, so those are not achievable at all and an option for them would be a lie. `kernel` is omitted for the mirror-image reason: `UpdateJob` accepts it on an ML Runtimes project where `CreateJob` refuses it, leaving a job carrying both a kernel and a runtime identifier. Two more shapes are encoded rather than documented away — `environment` is a JSON **string** here and an object on create (an object is `400 cannot unmarshal object into Go value of type string`), and `addonIdentifiers` without `runtimeIdentifier` throws before the wire, because alone it is a 500, `failed to fetch existing runtime for job`. Empty values differ per field: `arguments: ""` and `schedule: ""` clear (and the read-only `type` follows the schedule in both directions), while `environment: ""` reads as unset.
...
**Reads plus safe writes** — `files put`, `jobs create`, `jobs update`, `jobs run`, `runs stop`, `apps restart|stop`. **Nothing deletes anything.** The fence is the command surface, not a check: no verb reaches a destructive path, and `cai raw` accepts only GET. That last one lives in `src/lib/readonly.ts` with its own test rather than inline in the command, because "the escape hatch cannot write" is a security property and not a detail of one command's parsing. Adding a `cai * delete`, or widening `raw`, would undo the whole posture; `writes.test.ts` asserts that `apps delete` is simply not a command.

**`jobs create` decides which engine field is legal from the project, not from the flags.** An `ml_runtime` project requires `runtime_identifier` and a legacy-engine one requires `kernel`; the spec says so both ways and the command reads `default_engine_type` to refuse the wrong one up front rather than forwarding a 400 that says less. Four things about it were verified against a live instance on 2026-08-24 rather than read off the spec: the API **validates that the script exists** (`400 script 'x.py' not found in project directory`), `timezone` defaults to **`America/Los_Angeles`** even on a manual job — so a cron without one runs on Pacific time, which is why the command warns instead of substituting a timezone the caller never asked for — scheduled jobs are created **un-paused**, and the API **adds runtime addons that were not requested** (a job created with no `--addon` came back carrying `hadoop-cli-…`), so a caller must read `runtime_addon_identifiers` off the result. Also: the number in a CML UI job URL is not the API's job id, and the API rejects it with `Problem with field JobId: value length must be 19 runes`.

**`jobs update` treats a 200 as a claim to be checked, not as success.** Its two design choices both follow from what the API does rather than from what it says. First, the fields it will not forward: `--timezone` and `--paused` exist only to fail with the reason (the API ignores both, and there is no pause operation to fall back on), and there is no `--kernel` because updating one onto a runtime job is accepted and corrupting. Second, `lib/jobUpdate.ts` diffs the returned job against what was requested and warns about anything that did not land — no flag reaches the three known silent-no-op fields, so it should never fire, and that is the point: it is there for the field that joins them in the next firmware. Addons force one extra `GET` before the `PATCH` whenever only one half of the runtime/addon pair was given, because they cannot be sent alone (500) and a runtime sent alone resets them; carrying them over is announced rather than done quietly, the same rule that makes `files put` report the name the instance really stored. An update with no field flags is refused instead of sent — an empty body is a 200 that bumps `updated_at` and changes nothing.

**A job's `arguments` reach the script as the `JOB_ARGUMENTS` environment variable, not as argv.** The CML UI labels the field that way, and a real job's script confirms it — `sys.argv.extend(shlex.split(os.environ.get("JOB_ARGUMENTS", "")))`. A script reading only `sys.argv` receives nothing, so anything documenting this must not describe `arguments` as command-line arguments; `skills/cai/SKILL.md` and the README both state the variable.
...
**Nothing prints an `environment` blob unless asked, and `--json` is not the way around it.** CML injects its own variables into a project's environment — `CML_USER_PW` and `IAM_PASSWORD` among them — so the blob riding on every job, run and project is not only what its author set. Printing it by default put live credentials into a terminal, its scrollback, an agent's context window and a saved transcript at once, which is what issue #6 reported. `lib/sanitize.ts` reduces it to `"11 vars hidden — pass --show-env"`; `--show-env` masks credential-shaped names (`isSecretName`) and keeps everything else, because the *value* of `PYTHONPATH` is what a diagnosis often turns on; `--reveal` prints it verbatim and warns that it did. `isSecretName` matches whole *parts* rather than substrings — `TOKENIZER` and `PARTITION_KEY` are ordinary variables — and splits `camelCase` on its case boundary first, so `apiToken` is judged as `API_TOKEN` without widening the rule. `CML_SECRETS` names the platform's own injections outright: the patterns catch them today, but what a name looks like is a judgement that may be narrowed and what CML puts in every environment is not, so the list is the floor under the patterns rather than a duplicate of them. Four things about that are load-bearing. It hangs off `emit` in `BaseCommand` — the one seam every command's output passes through — because a per-command field list would be one command behind the next place CML attaches a blob, and because `cai raw` must not be the bypass either. A command must therefore **return what `emit` returned**, not the object it passed in: with `--json` oclif prints the return value itself, so `return job` after `this.emit(job, …)` hands the raw blob to exactly the caller the redaction is for. The marker is printed rather than the field dropped, since a reader who never sees the key learns the field does not exist and never finds the flag. And the shape is preserved — a JSON string in is a JSON string out under `--show-env` — so a script already parsing it needs no branch. `jobs update` reads the real job before `emit` for its unapplied-fields check, which is why that verification still works with the values hidden.

**`jobs run --wait` ends on `id status started finished`, not on the whole run.** Waiting is something the caller delegated, so what is owed back is the outcome; `runs get` is one command away for the rest, and that full print is what leaked in issue #6 while doing something nobody asked for. Without `--wait` the created run is still printed whole (redacted), because that is the API's answer to the caller's own write. Either way the id and the status survive, which is the constraint the whole feature is under: no script may ever need `--show-env` to function, or secrecy gets turned off globally and the default stops mattering.
...
**`skills/cai/SKILL.md` installs and updates itself from the CLI, not from `postinstall`.** `lib/skills.ts` runs in `BaseCommand.finally`, so every command ends by checking that the copy in `~/.claude/skills` is the one this version ships. Three things forced that design, and none of them is obvious:

- **npm 12 blocks lifecycle scripts by default** — they run only for packages named in `allow-scripts`, and the block is a *warning* on the installing user's terminal (`1 package had install scripts blocked…`), not an error. So `npm i -g @defysoftware/cai` succeeds while the skill is never written, and nobody finds out. Verified on npm 12.0.2 with no `allow-scripts` set anywhere. `postinstall` still runs where scripts are allowed and calls the same `syncSkills`, so the two paths cannot drift; it is an optimisation that gets the skill there before the first run, never the mechanism.
- **A stamp is what makes an upgrade land.** The old rule — leave anything that differs from the bundled copy alone — cannot tell an edited file from *last version's* file, so it wrote `SKILL.md.new` and kept the stale one on every upgrade. The skill installed once on a fresh machine and then never changed again. `.cai-skill.json` records the version plus the digest of what the CLI last wrote; a destination matching that digest is ours to replace, anything else is the user's to keep, and a file we have no record of is kept too, because unknown provenance is somebody else's work. The exception is a skill directory with **no stamp at all** (`planLegacyFile`): everything there predates stamps and therefore came from this CLI, so it is adopted rather than kept — otherwise every existing user stays frozen one upgrade short of the fix — and the copy it replaces is kept as `.bak`, since the one case where provenance cannot be proved is the one case worth a backup. From the first stamp onward the ordinary rule applies.
- **The development guard keys on `node_modules`, not on `INIT_CWD`.** A `node_modules` segment in the package path means an installation whatever git checkout sits above it; the old `INIT_CWD`-plus-`.git` test also skipped an ordinary `npm install` into any project that happened to be a git repository. An explicit `CAI_SKILLS_DIR` outranks the guard, since the guard exists to protect a directory nobody named — which is also how the tests exercise the real thing.

It never throws (a skill that could not be written must not fail a command that worked), reports on **stderr** so stdout stays parseable, and does nothing at all once the stamp names the running version — one small read per skill per command. `CAI_SKIP_SKILLS=1` opts out, `CAI_SKILLS_DIR` redirects, and `runCommand` in the tests sets `CAI_SKIP_SKILLS` by default so a test run never writes into the developer's own skills directory.
...
- **150-line soft limit per file.** Currently exceeded by `sessionManager.ts` (296), `sessionReconciler.ts` (243), `types.ts` (219), `extension.ts` (215), `sessionFormHtml.ts` (204, almost entirely markup), `sessionForm.ts` (195), `sshConfig.ts` (189), `sessionPanel.ts` (167), `runtimePicker.ts` (163), `sessionStatus.ts` (159), `cdswctl.ts` (157), `sessionFormData.ts` (154), `packages/cli/src/baseCommand.ts` (203), and in `packages/core/src` `operations/jobs.ts` (232 — two option types that document what the API does with each field), `operations/runs.ts` (171), `types.ts` (163), `http.ts` (157), `operations/projects.ts` (151), `session/cdswctl.ts` (190) and `session/sshConfig.ts` (171), plus in `packages/cli/src` `commands/session/create.ts` (262 — the endpoint-creation sequence, which the extension spends 312 lines on), `commands/jobs/update.ts` (214 — flags plus the runtime/addon pairing rule), `commands/jobs/create.ts` (166 — flags plus the engine-field rule), `lib/sanitize.ts` (213 — the deny-list and why each pattern is in it), `lib/skills.ts` (258 — the sync rules and the provenance reasoning behind each), `lib/tunnel.ts` (172) and `commands/session/kill.ts` (155); split by responsibility rather than growing these further. The two `media/` files are exempt — they are assets, not modules.
