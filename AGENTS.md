...
- **`package-lock.json` must resolve every package from `registry.npmjs.org`, and a step checks it.** `npm install` run on a machine whose `.npmrc` points at a private mirror rewrites the `resolved` URLs to that mirror; the runner cannot reach the host, so `npm ci` fails on a TLS error that names a certificate rather than the mirror, at step 4 of 18 with nothing else to go on. It is invisible locally — on that machine the host resolves — and it broke the 1.19.0 release, which is why the grep now runs *before* `npm ci` and says what to do. If your `npm config get registry` is not npmjs, regenerate with `npm install --registry=https://registry.npmjs.org` before committing a lockfile, or rewrite the mirror prefix out of the `resolved` URLs: the tarballs are byte-identical, so the `integrity` hashes stay valid and `npm ci` verifies them anyway.
