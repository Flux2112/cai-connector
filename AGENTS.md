...
- **150-line soft limit per file.** Currently exceeded by `sessionManager.ts` (296), `sessionReconciler.ts` (243), `types.ts` (219), `extension.ts` (215), `sessionFormHtml.ts` (204, almost entirely markup), `sessionForm.ts` (195), `sshConfig.ts` (189), `sessionPanel.ts` (167), `runtimePicker.ts` (163), `sessionStatus.ts` (159), `cdswctl.ts` (157), `sessionFormData.ts` (154), `packages/cli/src/baseCommand.ts` (183), and in `packages/core/src` `operations/jobs.ts` (232 — two option types that document what the API does with each field), `operations/runs.ts` (171), `types.ts` (163), `http.ts` (157), `operations/projects.ts` (151), `session/cdswctl.ts` (190) and `session/sshConfig.ts` (171), plus in `packages/cli/src` `commands/session/create.ts` (262 — the endpoint-creation sequence, which the extension spends 312 lines on), `commands/jobs/update.ts` (214 — flags plus the runtime/addon pairing rule), `commands/jobs/create.ts` (166 — flags plus the engine-field rule), `lib/sanitize.ts` (213 — the deny-list and why each pattern is in it), `lib/tunnel.ts` (172) and `commands/session/kill.ts` (155); split by responsibility rather than growing these further. The two `media/` files are exempt — they are assets, not modules.
